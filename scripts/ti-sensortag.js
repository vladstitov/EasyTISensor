...
var TISensorTag = (function()
{
	...
	sensortag.createInstance = function()
	{
		...
		instance.connectToDevice = function()
		{
			...
			instance.device.connect(
				function(device)
				{
				    ...
				    device.readServices(instance.requiredServices, instance.activateSensors, instance.errorFun);
				},
				instance.errorFun)
		}
		...
		instance.keypressOn = function(){
			instance.sensorOn(null,null, null, null, sensortag.KEYPRESS_DATA,sensortag.KEYPRESS_NOTIFICATION,instance.keypressFun)
		 ...
		 */
		instance.sensorOn = function(configUUID,configValue,periodUUID,	periodValue,dataUUID,notificationUUID,notificationFunction){
			// Only start sensor if a notification function has been set.
			if (!notificationFunction) { return }

			// Set sensor configuration to ON.
			configUUID && instance.device.writeCharacteristic(configUUID, new Uint8Array([configValue]), function () { }, instance.errorFun);

			// Set sensor update period.
			periodUUID && periodValue && instance.device.writeCharacteristic(periodUUID,new Uint8Array([periodValue / 10]),	function() {},instance.errorFun)

			// Set sensor notification to ON.
			if (dataUUID && notificationUUID) {               
			    instance.device.writeDescriptor(dataUUID,notificationUUID, 	new Uint8Array([1, 0]),	function () { },instance.errorFun)
			}
		...
		instance.sensorOff = function(dataUUID)	{
		    dataUUID && instance.device.disableNotification(dataUUID, function () { }, instance.errorFun);
			return instance
		}
