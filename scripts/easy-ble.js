...
var easyble = (function(){	
	...
	internal.readServices = function(device, serviceUUIDs, win, fail){
		...
		evothings.ble.services(device.deviceHandle,
			function (services) {
				...
				internal.readCharacteristicsForServices(device, serviceUUIDs, win, fail);
			},
			function(errorCode)
			{
				fail(errorCode);
	...
	internal.readCharacteristicsForServices = function(device, serviceUUIDs, win, fail)	{
		var characteristicsCallbackFun = function(service){
			...
			return function (characteristics) {
			    console.log('characteristics for service: ' + serviceUUIDs );
				...
				for (var i = 0; i < characteristics.length; ++i){
					...
					evothings.ble.descriptors(device.deviceHandle,characteristic.handle,descriptorsCallbackFun(characteristic),	function(errorCode)	{	fail(errorCode);});
				}
			};
		};
		...
		if (null != serviceUUIDs){
			...
			for (var i = 0; i < serviceUUIDs.length; ++i){
				...
				evothings.ble.characteristics(device.deviceHandle,	service.handle,	characteristicsCallbackFun(service),	function(errorCode)	{fail(errorCode);});
			}
		}
		else
		{
	...
	internal.readCharacteristic = function(device, characteristicUUID, win, fail){
		...
		evothings.ble.readCharacteristic(device.deviceHandle,characteristic.handle,	win,	fail);
	};
	...
	internal.writeCharacteristic = function(device, characteristicUUID, value, win, fail){
		...
		evothings.ble.writeCharacteristic(device.deviceHandle,characteristic.handle,value,	function(){	win();},function(errorCode)	{fail(errorCode);});
	};
	...
	internal.writeDescriptor = function(device, characteristicUUID, descriptorUUID, value, win, fail){
		...
		evothings.ble.writeDescriptor(	device.deviceHandle,descriptor.handle,	value,	function()	{win();	},function(errorCode)	{	fail(errorCode);});
	};
	...
	internal.enableNotification = function(device, characteristicUUID, win, fail){
		...
		console.log('enableNotification', device.deviceHandle, characteristic.handle);
		
		evothings.ble.enableNotification(device.deviceHandle,characteristic.handle,	win,fail);
	};
	...
	internal.disableNotification = function(device, characteristicUUID, win, fail){
		...
		console.log('disableNotification', device.deviceHandle, characteristic.handle);
		evothings.ble.disableNotification(	device.deviceHandle,characteristic.handle,	win,fail);
	};
